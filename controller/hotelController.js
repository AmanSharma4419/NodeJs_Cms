...
module.exports = {
    ...
    getAllHotelList: async (req, res) => {
        try {
            ...
            let count = await Hotel.aggregate([{$match:obj},{ $group: { _id: null, count: { $sum: 1 } } }]);
            Hotel.aggregate(
                [
                  {$match:obj},
    ...
    registerHotelByAdmin: async (req, res) => {
        try {
            var data = req.body;
            ...
            var checkHotelName = await Hotel.getHotelByName({ name: data.name });

            if (checkHotelName != null) {
                return res.status(400).json({ "status": "failure", "message": "Hotel Name is already exist!" });
            }

            if (!data.phoneNumber) {
                return res.status(400).json({ "status": "failure", "message": "Phone Number is required!" });
            }
            var checkHotelEmail = await Hotel.getHotelByEmail({ email: data.email });
            if(checkHotelEmail != null){
                return res.status(400).json({ "status": "failure", "message": "Hotel email already exist already exist!" });
            }
            if (!data.email) {
            ...
            data.hotelLocation = { type: "Point", coordinates: [data.lng, data.lat] };

            Hotel.addHotel(data, (err, resdata) => {
                if (err || resdata == null) {
                    return res.status(400).json({ "status": "failure", "message": "Unable to register hotel!", "data": {}, "error": err });
                } else {
                    res.status(200).json({ "status": "success", "message": "Hotel registered successfully!", "data": resdata });
                }
            });
    ...
    hotelLoginAuth: async (req, res) => {
        try {
            ...
            var verify = await Hotel.getHotelByEmail({ email: data.email });
            ...
            Hotel.updateTokenHotel(data, function (err, tokendata) {
    ...
    getHotelProfileDetails: async (req, res) => {
        try {
            var verifydata = await Auth.validateHotel(req.headers);
            if (verifydata == null) {
                return res.status(401).json({ message: "Not Authorized!", data: {}, error: {} })
            }

            Hotel.getHotelById(verifydata._id, (err, resdata) => {
                if (err || resdata == null) {
                    return res.status(400).json({ "message": "Not a valid Hotel!", "data": {}, "error": err })
                }
                else {
                    res.json({ "message": "Hotel Details!", "data": resdata, "error": {} })
                }
            });
    ...
    getHotelById: async (req,res) => {
        try{
            ...
            if(!req.params.hotelId){
                return res.status(200).json({ "message": "Hotel id is required.", "data": {}, "error": {} });
            }
            Hotel.getHotelById(req.params.hotelId, (err, resdata) => {
                if(err){
                    return res.status(400).json({ "message": "Not a valid Hotel!", "data": {}, "error": err })
                }else{
                    res.json({ "message": "Hotel Details!", "data": resdata, "error": {} })
    ...
    hotelUpdateProfile: async (req, res) => {
        try {
            var verifydata = await Auth.validateHotel(req.headers);
            var verifydataadmin = await Auth.validateAdmin(req.headers);
            ...
            Hotel.updateProfile(data, (err, resdata) => {
                if (err || resdata == null) {
                    return res.status(400).json({ "message": "Not a valid Hotel!", "data": {}, "error": err })
                }
                else {
    ...
    removeHotelByAdmin: async (req, res) => {
        try {
            var data = req.body;
            ...
            Hotel.removeHotel(data.hotelId, (err, resdata) => {
                if (err || resdata == null) {
                    return res.status(400).json({ "status": "failure", "message": "Wrong Dispatcher id!", data: {}, "error": err });
                }
                else {
                    res.json({ "status": "success", "message": "Hotel deleted successfully!", data: resdata, "error": {} });
                }
            });
    ...
    fmChangePassword: async (req, res) => {
        try {
            ...
            Hotel.updatePasswordHotel(data, async function (err, resdata) {
                if (err || resdata == null) {
    ...
    fmForgotPassword : async(req,res) => {
        try {
          ...
          var verifyemail = await Hotel.findHotel(data.email);
          ...
          Hotel.updateOTPHotel(data, function (err, resdata) {
              if (err || resdata == null) {
    ...
    fmResetPassword : async(req,res) => {
        try {
          ...
          Hotel.updatePasswordHotelPanle(data, async function (err, resdata) {
              if (err || resdata == null) {
                  return res.status(400).json({ "message": "Can not update password", "data": {}, "error": err })
              }
    ...
    hotelCalculateEstimateCost: async (req, res) => {
        try {
            var verifydata = await Auth.validateHotel(req.headers);
            if (verifydata == null) {
                return res.status(401).json({ message: "Not Authorized!", data: {}, error: {} })
    ...
    hotelCreateCustomerRequest: async (req, res) => {
        try {
            var data = req.body;
    ...
    addUserTripInstant: async (data, headerData, responseCallback) => {
        ...
        try {
            ...
            helper.checkRequestParams(data, paramsList, async (response) => {
                if (response.status) {
                    var verifydata = await Auth.validateHotel(headerData);
                    if (verifydata == null) {
                    ...
                    data.hotelId = verifydata._id;
                    data.hotelIdRef = verifydata._id;
                    ...
                    data.customerAvgRating = verifydata.hotelAvgRating;
                    ...
                    var addHotelCustomer = await User.addHotelCustomer(cdata);
                    data.customerId = addHotelCustomer._id;
                    data.customerRefId = addHotelCustomer._id;
                    ...
                    tripModel.addTripInstantHotel(data, async (err, datad) => {
                        if (err) {
            ...
            helper.checkRequestParams(data, paramsList, async (response) => {
                if (response.status) {
                    ...
                    if (resultInMinutes < 31) {
                        ...
                        tripModel.addScheduleHotelTrip(data, async (err, datad) => {
                            if (err) {
                                responseCallback(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                            }
                            else {
    ...
    getHotelRideNowRequest: async (req, res) => {
        try {
            var verifydata = await Auth.validateHotel(req.headers);
            if (verifydata == null) {
                return res.status(401).json({ message: "Not Authorized!", data: {}, error: {} })
            ...
            let count = await trips.aggregate([{$match:obj},{$match:{hotelIdRef:ObjectId(verifydata._id)}},{$match:{trip_type: "ridenow"}},{ $group: { _id: null, count: { $sum: 1 } } }]);
            trips.aggregate([
                {$match:{hotelIdRef:ObjectId(verifydata._id)}},
                {$match:obj},
    ...
    getHotelRideLaterRequest: async (req, res) => {
        try {
            var verifydata = await Auth.validateHotel(req.headers);
            console.log("Data", req.body);
            if (verifydata == null) {
            ...
            console.log("Filter Object", obj);
            let count = await trips.aggregate([{$match:obj},{$match:{hotelIdRef:ObjectId(verifydata._id)}},{$match:{trip_type: "ridelater"}},{ $group: { _id: null, count: { $sum: 1 } } }]);
    ...
    getHoterTripDeatails: async (req, res) => {
        try {
            var verifydata = await Auth.validateHotel(req.headers);
            if (verifydata == null) {
                return res.status(401).json({ message: "Not Authorized!", data: {}, error: {} })
    ...
    getTripsByStatus: async (req,res) =>{
      try{
        ...
        let count = await trips.aggregate([{$match:{hotelIdRef:ObjectId(verifydata._id)}},{$match:obj},{ $group: { _id: null, count: { $sum: 1 } } }]);
        trips.aggregate([
            {$match:{hotelIdRef:ObjectId(verifydata._id)}},
            {$match:obj},
            {$sort :{[sortByField]: parseInt(sortOrder)}},{$skip: (paged-1)*pageSize},
     ...
     cancelHotelTripRequest : async(req,res) => {
        try {
            var verifydata = await Auth.validateHotel(req.headers);
            if (verifydata == null) {
                return res.status(401).json({ message: "Not Authorized!", data: {}, error: {} })
    ...
    getTripById: async(req,res) => {
         try{
            ...
            trips.getHotelTripById(tripId, async (err, datad) => {
                if (err || datad == null) {
