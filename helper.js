...
module.exports = {
    ...
    showDatabaseErrorResponse: (message, err) => {
        var resData = { "status": "failure", "status_code": 200, "error_code": 5005, "error_description": "Database error!", "message": __(message), "data": {}, "error": err };
        return resData;
    },

    showAWSImageUploadErrorResponse: (message, err) => {
        var resData = { "status": "failure", "status_code": 200, "error_code": 5006, "error_description": "AWS error!", "message": __(message), "data": {}, "error": err };
        return resData;
    },
    ...
    showStripeErrorResponse: (message) => {
        var resData = { "status": "failure", "status_code": 200, "error_code": 5009, "error_description": "Stripe Api Error!", "message": message, "data": {}, "error": {} };
        return resData;
    },
    ...
    caculateTripEstimatedCost: (distance, time, element) => {
        ...
        if (element.unit === "km") {
            ...
            unit = 'km';
            estimatedCost = Math.round(totalPrice * 100) / 100;
            ...
            if (isSurgeTime === "yes") {
                estimatedCost = Math.round((estimatedCost * surgeMultiplier) * 100) / 100;
            }

        } else if (element.unit === "mile") {

            distance = distance * 0.000621371192; //distance in miles
            var baseDistancePrice = element.distanceForBaseFare * element.basePrice;
            var leftDistance = distance - element.distanceForBaseFare;
            var leftDistancePrice = element.pricePerUnitDistance * leftDistance;
            var totalDistancePrice = baseDistancePrice + leftDistancePrice;
    ...
    sendCompleteTripEmailToDriverAndCustomer: async (costbeforPromo, datad) => {
        ...
        if (eBody != null) {
            ...
            try {
                var msg = eBody;
                var customere = await User.findOne({ mobileNumber: datad.customerNumber });
    ...
    getDistanceBetweenLocation: (startLocation, endLocation)=>{
