...
const errmodel = require('../models/errortable');
...
module.exports = {
    ...
    sendCompleteTripEmailToDriverAndCustomer: async (costbeforPromo, datad) => {
        ...
        if (eBody != null) {
            ...
            try {
                var msg = eBody;
                var customerecust = await User.findOne({ _id: datad.customerId.toString() });

                if(customerecust != null){
                    var senemail = await mailgunSendEmail.sendEmail(customerecust.email, __('TRIP_COMPLETE_EMAIL_SUBJECT', constant.APP_NAME), msg);
    ...
    checkLocationExitsBetweenSourceDestination: (point, lineStart, lineEnd) => {
        ...
        var checkLatLng = geolib.isPointInLine(
            ...
            { latitude: lineStart.lat, longitude: lineStart.lng },
            { latitude: lineEnd.lat, longitude: lineEnd.lat }
        );

        //console.log("checkLatLng", checkLatLng);

        return checkLatLng;
    },

    checkLocationBetweenRadius : (point, centerPoint) => {

        var checkPoint = geolib.isPointWithinRadius(
            { latitude: point.lat, longitude: point.lng },
    ...
    sortDriverPoolTrips: async(driverId,driverLocation) => {

        var fromLocation = [];
        fromLocation.push(driverLocation.coordinates[1]);
        fromLocation.push(driverLocation.coordinates[0]);

        var tripList = [];
        //isPointInLine(point, lineStart, lineEnd);
        var getDriverTrips = await Trips.getDriverCurrentPoolTrips(driverId);
        ...
        await Promise.all(getDriverTrips.map(async element => {
            ...
            if(element.tripStatus === constant.TRIP_DESTINATION_INROUTE){

                var toLocationEnd = [];
                toLocationEnd.push(element.endLocation.coordinates[1]);
                toLocationEnd.push(element.endLocation.coordinates[0]);

                var getDistanceByGoogle = await helper.calculateDistanceByGoogleApi(fromLocation,toLocationEnd);

                console.log("getDistanceByGoogle",getDistanceByGoogle);

                var distance2 = 0;
                if(getDistanceByGoogle === 0){
                     distance2 = helper.getDistanceFromTwoLocation(fromLocation,toLocationEnd);
                }else{
                     distance2 = getDistanceByGoogle;
                }

                var  tripObject2 = {
                    tripId : element._id,
                    distance: helper.roundNumber(distance2)
                };

                tripList.push(tripObject2);

            }else{

                //calculate distance
                var toLocationStart = [];
                toLocationStart.push(element.startLocation.coordinates[1]);
                toLocationStart.push(element.startLocation.coordinates[0]);
                
                var getDistanceByGoogle = await helper.calculateDistanceByGoogleApi(fromLocation,toLocationStart);

                console.log("getDistanceByGoogle1",getDistanceByGoogle);
                
                var distance1 = 0;
                if(getDistanceByGoogle === 0){
                     distance1 = helper.getDistanceFromTwoLocation(fromLocation,toLocationStart);
                }else{
                     distance1 = getDistanceByGoogle;
                }

                var  tripObject1 = {
                    tripId : element._id,
                    distance: helper.roundNumber(distance1)
                };

                tripList.push(tripObject1);
        ...
        console.log("tripList222", tripList);

        return tripList[0].tripId;

    },

    calculateDistanceByGoogleApi : async(start,end) => {

        var params = {
            origins: { lat: start[0], lng: start[1] }, //"Washington, DC, USA"
            destinations: { lat: end[0], lng: end[1] }, //"New York, NY, USA"
            mode: "driving"
        }
